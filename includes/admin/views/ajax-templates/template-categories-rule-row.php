...
if ( isset( $_POST['_wcol_new_rule_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wcol_new_rule_nonce'] ) ), 'wcol_new_rule' ) ) {
	if ( isset( $_POST['wcol_cid'] ) && ! empty( $_POST['wcol_cid'] ) ) {
		$cid = sanitize_text_field( wp_unslash( $_POST['wcol_cid'] ) );
	}
}
?>
<tr class="wcol-new" data-id="<?php echo esc_attr( $rule_id ); ?>">
	<td class="check-column wcol-cb">
		<input type="checkbox"/>
		<input type="hidden" name="wcol-rules[category-rules][rule_id][<?php echo esc_html( $cid ); ?>]" value="<?php echo esc_html( $rule_id ); ?>">
	</td>
	<td class="column-primary">
		<select class="wcol-select-categories" name="wcol-rules[category-rules][object-ids][<?php echo esc_html( $cid ); ?>][]" multiple="multiple"></select>
	...
	<td data-colname="<?php esc_attr_e( 'Minimum Limit', 'wc-order-limit-lite' ); ?>">
		<input type="number" min="0" class="wcol-rule-min-limit" name="wcol-rules[category-rules][rule-limit][<?php echo esc_html( $cid ); ?>]" value="0" />
	</td>
	...
	<td data-colname="<?php esc_attr_e( 'Applied On', 'wc-order-limit-lite' ); ?>">
		...
		?>
		<select class="wcol-select-applied-on" name="wcol-rules[category-rules][applied-on][<?php echo esc_html( $cid ); ?>]" >
			<?php
            // phpcs:ignore
	...
	<td data-colname="<?php esc_attr_e( 'Accumulatively', 'wc-order-limit-lite' ); ?>">
		<input type="hidden" class="wcol-loop-checkbox-hidden" name="wcol-rules[category-rules][accomulative][<?php echo esc_html( $cid ); ?>]"/>
			...
			<div class=" wcol-rule-options wcol-hidden">
				...
				<table class="">
					<tr>
						...
						<td>
							<input type="hidden" class="wcol-loop-checkbox-hidden" name="wcol-rules[category-rules][disable-limit][<?php echo esc_html( $cid ); ?>]" />
					...
					<tr>
						...
						<td>
							<input type="hidden" class="enable-max-rule-limit-hidden wcol-loop-checkbox-hidden" name="wcol-rules[category-rules][enable-max-rule-limit][<?php echo esc_html( $cid ); ?>]"/>
					...
					<tr class="wcol-hidden">
						...
						<td><input type="number" min="0" class="wcol-rule-max-limit" name="wcol-rules[category-rules][max-rule-limit][<?php echo esc_html( $cid ); ?>]" /></td>
					</tr>
